...
function anagrams(stringA, stringB) {
  ...
  // sanitize input strings to account for case insensitivity and non-alpha-numeric charaters
  // use .replace() with regular expressions so that only letter characters remain
  // use .toLowerCase() for case insensitivity
  stringA = stringA.toLowerCase().replace(/[\W_]+/g, "");
  stringB = stringB.toLowerCase().replace(/[\W_]+/g, "");

  // if input string do not have the same length, return false immediately
  if (stringA.length !== stringB.length) {
    return false;
  }

  // create a character count object for one of the input strings
  ...
  // loop through the other input string, decrementing charater count object
